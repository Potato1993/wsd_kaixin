...
//Start connection to datadb pessdb
require_once 'db.php';
...
while ($row = $result -> fetch_assoc()) {
  ...
  $id = $row['patrolcar_id'];
  // create var status to contain the column value patrolcar_status_desc ofarow
  $status = $row[ 'patrolcar_status_desc'];
  // create array var car to contain the column values ofarow
  $car = ["id" => $id, "status" => $status]; 
  // using the array_push function to assign all rows of the result-set into array var cars
  array_push($cars, $car);   
}

//create var btnDispatchClicked to check if btnDispatched button has been clicked
...
if ($btnDispatchClicked == false && $btnProcessCallClicked == false) {
    //use hesader function to append error message in URL
    header("Location: logcall.php?message=error");
}

// if btnDispatch has been clicked
if ($btnDispatchClicked == true){
  // create var insertīncidentSuccess with initial value false
            ...
            <form action="dispatch.php" method="post">
                 ...
                 <div class="form-group row">
                     ...
                     <div class="col-sm-8">
                        ...
                        $result = $conn -> query($sql);
                        // usingawhile loop to extract incident_type_desc from incident_type table
                        while ($row = $result -> fetch_assoc()){
                 ...
                 <div class="form-group row">
                  ...
                  <!-- Row to display patrol car to dispatch -->
                 <div class="form-group row">
                     <label for="patrolCars" class="col-sm-4 col-form-label"> Choose Catrol Car(s) </label>
                     <div class="col-sm-8">
                         <table class="table table-striped">
                            <tbody>
                                ...
                                <?php
                                //use for loop to populatw the table row with patrolcar details retrived from array var cars
